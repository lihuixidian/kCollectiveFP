...
geo_point geo_offset(const geo_point& gp, long double distance, direction dir)//该函数的功能到底是什么让gp按照某一个方向平移一段距离？？？？？
{
	geo_point offset_gp;
	if (dir == northward || dir == southward)	// The same longitude.经度
	{
	...
	else if (dir == eastward || dir == westward)	// The same latitude.纬度
	{
