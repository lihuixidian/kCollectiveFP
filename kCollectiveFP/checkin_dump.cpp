...
// Each check-in is a line, with '\t' as seperator between user (string), lo/la (long double) and la/lo (long double) fields.
// [IN] data_file_path : Check-in data file full path.文件中有用户名称，但是只将它的位置点读出
		...
		while (!in_checkin_file.bad() && in_checkin_file.good())
		{
			...
			if (user_longlat_pos != std::string::npos)
			{
				...
				if (long_lat_pos != std::string::npos)
				{
					...
					if (is_long_prior_to_lat)
					{
						longitude = std::stold(str_buf.substr(user_longlat_pos + 1, long_lat_pos - user_longlat_pos - 1));//将字符转为浮点数
...
// Each coordinate is a line, with '\t' as seperator between lo/la (long double) and la/lo (long double) fields.
// [IN] data_file_path : Candidate data file full path.
// [OUT] candidate_map : A coordinate (checkin::geo_point) map to store candidates.从文件中读取位置的经纬度，文件中没有用户信息
void checkin_dump::load_string_coordinates(const char* data_file_path, std::vector<geo_coordinate>& candidate_vector)
{
	...
	{
		while (!in_candidate_file.bad() && in_candidate_file.good())//检查是否发生错误
		{
			char buf[1024];
											 ...
											 long double& min_long_dist, long double& max_long_dist, long double& ave_long_dist)
{
	...
	std::ofstream out_least_file(least_data_file_path, std::ofstream::out | std::ofstream::trunc);	
// File will be closed when ofstream destroyed.
...
//将每个用户的位置个数做了区分，分别写入不同的文件中，并且记录了每种文件中的用户个数
											 ...
											 long double& min_long_dist, long double& max_long_dist, long double& ave_long_dist)
{
	...
	if (!out_least_file.fail() && !out_less_file.fail() && !out_normal_file.fail() && !out_more_file.fail() && !out_most_file.fail())	// File opened or created.
	{
		...
		{
			long double temp_lat_dist, temp_long_dist, temp_min_lat, temp_max_lat, temp_min_long, temp_max_long;
			if (iter_map->second.size() < 10)//将每个用户的位置个数做了区分，分别写入不同的文件中，并且记录了每种文件中的用户个数
			{
				++num_least;
				dump_and_cal(out_least_file, iter_map->first, iter_map->second, temp_lat_dist, temp_long_dist, temp_min_lat, temp_max_lat, temp_min_long, temp_max_long);
			}
			...
			if (temp_min_lat < min_lat)//找到，最小经纬度，和最大经纬度
				min_lat = temp_min_lat;
			if (temp_max_lat > max_lat)
				max_lat = temp_max_lat;
			if (temp_min_long < min_long)
		...
		lat_dist = geo_distance(geo_point(min_long, min_lat), geo_point(min_long, max_lat));//相同经度下，纬度之间的距离
		long_dist = geo_distance(geo_point(min_long, min_lat), geo_point(max_long, min_lat));//相同纬度下，经度之间的距离
		ave_lat_dist = sum_lat_dist / checkin_map.size();//平均纬度距离
		ave_long_dist = sum_long_dist / checkin_map.size();//平均经度距离
	}
}

//将某些信息写入out_file所指向的文件中
								...
								long double& min_lat, long double& max_lat, long double& min_long, long double& max_long)
{
	...
	{
		if (iter->get<0>() < min_long)//遍历用户位置，找到min_long,max_long,min_lat,max_lat
			min_long = iter->get<0>();
		...
		out_file << user << '\t';//将用户信息写入文件
		if (is_long_prior_to_lat)//将位置信息写入文件
		{
			out_file << iter->get<0>() << '\t';
			out_file << iter->get<1>() << '\n';
		}
	...
	long_dist = geo_distance(geo_point(min_long, min_lat), geo_point(max_long, min_lat));//同纬度下，经度之间的距离
	lat_dist = geo_distance(geo_point(min_long, min_lat), geo_point(min_long, max_lat));//同经度下，纬度之间的距离
}
...
//从源文件中读取用户信息，然后随机选择num个用户到chosen_checkin_map中
//将chosen_checkin_map中的用户信息写入新的文件夹中
...
//重新归类登记用户位置个数，相当于是从多于num_pos_upper个位置的用户中，选取num_pos_upper-m*num_pos_decresed_by个位置写入不同的文件中
//num_user表示位置个数多于num_pos_upper的用户数量
void checkin_dump::load_dump_based_on_positions(const char* data_file_path, const char* pos_main_file_name, unsigned num_pos_upper, int num_pos_decreased_by, int& num_user)
{
	...
	std::map<std::string, std::vector<geo_point>>::const_iterator iter_map = checkin_map.cbegin();
	for (; iter_map != checkin_map.cend(); ++iter_map)//若该用户的位置数量多于num_pos_upper,就将该用户信息加入pos_map中
	{
	...
	for (int i = num_pos_upper; i >= num_pos_decreased_by; i -= num_pos_decreased_by)
	{
		...
		if (!out_file.fail())	// File opened or created.
		{
			...
			for (; iter_pos_map != pos_map.cend(); ++iter_pos_map)
			{
				...
				random(iter_pos_map->second, vector_user_pos, i);	// Random positions.随机取i个位置

				std::vector<geo_point>::const_iterator iter = vector_user_pos.cbegin();//将位置信息写入目标文件pos_main_file_name中
...
{
	obj.clear();
	std::vector<geo_point> residual(src);//创建了一个src的一个拷贝
	srand(static_cast<unsigned>(time(NULL)));
...
void checkin_dump::distances(const char* data_file_path, long double& min, long double& max, long double& ave, bool bCoordinateOnly)
{
	...
	for (unsigned i = 0; i < coordinates_vector.size(); ++i)//计算所有位置之间的距离，找出距离最大和最小的值，并且计算所有位置距离的平均值
	{
...
//寻找候选地位置与用户位置重复个数较多的候选地，前十个，前二十个，前三十个，前四十个，前五十个
										 ...
										 const char* top20_file_path, const char* top30_file_path, const char* top40_file_path, const char* top50_file_path)
{
	...
	if (!out_file_top10.fail() && !out_file_top20.fail() && !out_file_top30.fail() && !out_file_top40.fail() && !out_file_top50.fail())	// File opened or created.
	{
		...
		while (top_pick < 50)
		{
			...
			{
				if (iter->second == max_inf)	// Current max.一次寻找最大的
				{
					if (is_long_prior_to_lat)
					{
						if (top_pick < 10)
...
//找到候选地位置坐标与用户相同到的地方，并且记录相同数量,并且返回最大数量
											...
											int& max_inf)
{
	...
	for (; iter_vec != candidate_vector.cend(); ++iter_vec)	// For each candidate.
	{
		...
		candidate_result_map[*iter_vec] = real_checkin;//找到候选地位置坐标与用户相同到的地方，并且记录相同数量
...
void checkin_dump::APatK(const char* real_file_path, const char* query_file_path, double& ap, double& precision, double& recall)
{
	...
	if (!in_query_file.fail())	// File exists.
	{
		while (!in_query_file.bad() && in_query_file.good())
		{
			...
			if (long_lat_pos != std::string::npos)
			{
				...
				query_map[rank++] = geo_coordinate(longitude, latitude);//这里相当于将各个算法结果文件中读取相应的结果信息
			}
		}
	}
	...
	for (; iter_query != query_map.end(); ++iter_query)	//遍历query_map的结果
	{
		...
		for (; iter_real != real_map.end(); ++iter_real)
		{
			...
			if (iter_real->second == iter_query->second)//若query和real的结果相同
			{
				hit_sum++;//相同个数加1 
	...
	precision = static_cast<double>(hit_sum) / static_cast<double>(query_map.size());//两个结果中相同元素的个数在查询结果中站的比重
	recall = static_cast<double>(hit_sum) / static_cast<double>(real_map.size());//在真是结果中占的比重
	...
	{
		if (iter_ap->second == 1)//若是结果相同的元素
		{
			++hit;
			ap += static_cast<double>(hit) / static_cast<double>(iter_ap->first);
		}		
	}
	ap /= static_cast<double>(ap_map.size());//平均
...
// Transform my candidates file for MaxBRNN format.在候选地位置之前加了一个1
	...
	if (!out_file.fail())	// File opened or created.
	{
		...
		for (; iter_map != checkin_map.cend(); ++iter_map)
		{
			...
			if (iter_map->second.size() <= count)
				...
				random(iter_map->second, randomResult, count);//从用户位置个数多于24，中随机挑选24个位置

			std::vector<geo_point>::const_iterator iter = randomResult.cbegin();
			for (; iter != randomResult.cend(); ++iter)
			{
				out_file << iter_map->first << '\t';//将用户名写入outfile
				if (is_long_prior_to_lat)//将坐标写入file
				{
					out_file << iter->get<0>() << '\t';
					out_file << iter->get<1>() << '\n';
				}
